...
CKServerGeneralPage::CKServerGeneralPage ()
	...
	m_strRemoteMachine = _T("127.0.0.1");
	m_pServerList = NULL;
	}
...
/*
		...
		COSERVERINFO loServerInfo = {0};

		loServerInfo.pwszName = T2W(m_strRemoteMachine);
	

		MULTI_QI   mqi   [1] = {0}  ;

		mqi   [0].pIID   =   &IID_ICatInformation   ; 
		mqi   [0].pItf   =   NULL   ; 
		mqi   [0].hr   =   0   ; 


		hr = CoCreateInstanceEx(CLSID_StdComponentCategoriesMgr, 
			NULL,
			CLSCTX_SERVER, &loServerInfo,
			1,
			mqi);
		...
		if (SUCCEEDED (hr))
		{
			pCat = (ICatInformation *)mqi   [0].pItf;
...
//#ifdef _UNICODE
						USES_CONVERSION;
						m_pServerList->InsertItem(W2CT(wszProgID), ILI_COMPONENT, ILI_COMPONENT, hParent);
//#else
...
//#ifdef _UNICODE
			//lstrcpyn (_T(szBuffer), _T(pServerStatus->szVendorInfo), sizeof (szBuffer) / sizeof (TCHAR));
//#else
//			_wcstombsz (szBuffer, pServerStatus->szVendorInfo, sizeof (szBuffer) / sizeof (TCHAR));
//#endif
			// Add vender name:
			USES_CONVERSION;
			strLoader.FormatMessage(IDS_VENDORNAME, W2T(pServerStatus->szVendorInfo));
			strStatus += strLoader;
...
int CKServerPropertySheet::DoModal () 
	...
	if (nRC == IDOK)
		...
		if (m_pServer)
			{
			CKApp * lpApp = (CKApp *)AfxGetApp();
		//	m_pServer->AuthIdentityInfo(lpApp->GetAuthIdentity());
			m_pServer->Connect (
	...
	void CKServerGeneralPage::OnBnClickedButton1()
	{
		m_pServerList->DeleteAllItems();

		m_pServerList->UpdateWindow();

		this->Refresh();
	}

	void CKServerGeneralPage::Refresh()
	{
