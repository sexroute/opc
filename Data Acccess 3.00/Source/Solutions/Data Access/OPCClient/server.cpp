...
//  none
// **************************************************************************
CString CKServer::m_strPassword = _T("");
CString CKServer::m_strUserName = _T("");
...
void CKServer::AddGroup(CKGroup *pGroup, bool bLoadingProject /*= false */)
{
	...
	if (m_bConnected)
	{
		...
		if (SUCCEEDED(hr))
		{
			CKServer::AdjustPrivileges(pUnknown);
			// Log success:
...
void CKServer::Password(CString val)
{
	m_strPassword = val;
}

CString CKServer::Password() 
{
...
bool CKServer::Connect()
{
	...
	USES_CONVERSION;
	WCHAR *m_strName = T2W(CKServer::m_strUserName);
	WCHAR *m_strPassword = T2W(CKServer::m_strPassword);
	COAUTHIDENTITY us = { 0 };
	us.User = (USHORT *)m_strName;
	us.UserLength = wcslen(m_strName);
	us.Password = (USHORT *)m_strPassword;
	us.PasswordLength = wcslen(m_strPassword);
	us.Domain = NULL;
	us.DomainLength = 0;
	...
	if (SUCCEEDED(GetCLSID(clsid)))
	{
		...
		else
		{
			...
			ZeroMemory(tCoServerInfo.pwszName, nSize * sizeof(WCHAR));
			/*
						wcscpy(tCoServerInfo.pwszName,T2W(this->m_strRemoteMachine));
			...
			/*
						...
						{
							tCoServerInfo.pAuthInfo = &this->m_oAuthinfo;
						}*/

			tCoServerInfo.pAuthInfo = &auth;
			// CoCreateInstanceEx will launch the OPC Server if necessary, and
			// call its QueryInterface for us (bumping its reference count):
			hr = CoCreateInstanceEx(
				clsid,										// CLSID
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCSERVER].hr))
			{
				...
				CKServer::AdjustPrivileges(m_pIServer, us);
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCCOMMON].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pICommon, us);
				}
			}
			else
			{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCCONNPT].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pIConnPtContainer, us);
				}
			}
			else
			{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCITEMPROP].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pIItemProps, us);
				}
			}
			else
			{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCBROWSE].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pIBrowse, us);
				}
			}
			else
			{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCPUBLIC].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pIPublicGroups, us);
				}
			}
			else
			{
			...
			if (SUCCEEDED(m_arrMultiQI[MQI_IOPCPERSIST].hr))
			{
				...
				{
					CKServer::AdjustPrivileges(m_pIPersistFile, us);
				}
			}
			else
			{
			...
			if (m_pIConnPtContainer != NULL)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					CKServer::AdjustPrivileges(pCP, us);
...
bool CKServer::AdjustPrivileges(IUnknown * apData, COAUTHIDENTITY& arefIdentityInfo)
{
	return true;
	if (NULL == apData)
	{
	...
	HRESULT	hres = CoSetProxyBlanket(
		apData,                           // Indicates the proxy to set
		RPC_C_AUTHN_DEFAULT,            // RPC_C_AUTHN_xxx
		RPC_C_AUTHZ_DEFAULT,            // RPC_C_AUTHZ_xxx
		COLE_DEFAULT_PRINCIPAL,         // Server principal name 
		RPC_C_AUTHN_LEVEL_PKT_PRIVACY,  // RPC_C_AUTHN_LEVEL_xxx 
		RPC_C_IMP_LEVEL_IMPERSONATE,    // RPC_C_IMP_LEVEL_xxx
		&arefIdentityInfo,                       // client identity
...
bool CKServer::AdjustPrivileges(IUnknown * apData)
{
	USES_CONVERSION;
	WCHAR *m_strName = T2W(CKServer::m_strUserName);
	WCHAR *m_strPassword = T2W(CKServer::m_strPassword);
	...
	return CKServer::AdjustPrivileges(apData, us);
}

void CKServer::UserName(CString val)
{
	m_strUserName = val;
}

CString CKServer::UserName() 
{
