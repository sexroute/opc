			...
			while ((hr = pEnum->Next (1, &pIUnknown, &uFetched)) == S_OK)
				...
				if (SUCCEEDED (hr))
					...
					if (SUCCEEDED (hr))
						...
						// to buffer:
//#ifdef _UNICODE
						//lstrcpyn (_T(szBuffer), _T(pszName), sizeof (szBuffer) / sizeof (TCHAR));
//#else
//						_wcstombsz (szBuffer, pszName, sizeof (szBuffer) / sizeof (TCHAR));
//#endif
						// Insert group name into the list control:
						USES_CONVERSION;
						m_pEnumList->InsertItem (m_pEnumList->GetItemCount (), W2CT(pszName));
	...
	// Else enumerate by name (generally faster):
		...
		if (SUCCEEDED (hr) && pEnum != NULL)
			...
			while ((hr = pEnum->Next (1, &pString, &uFetched)) == S_OK)
				...
				// to buffer:
//#ifdef _UNICODE
				//lstrcpyn (_T(szBuffer), _T(pString), sizeof (szBuffer) / sizeof (TCHAR));
//#else
...
//#endif
				...
				// Insert group name into the list control:
				USES_CONVERSION;
				m_pEnumList->InsertItem (m_pEnumList->GetItemCount (), W2CT(pString));
