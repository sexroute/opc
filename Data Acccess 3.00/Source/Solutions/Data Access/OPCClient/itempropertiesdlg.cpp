...
//#ifdef _UNICODE
						USES_CONVERSION;
						pList->SetItemText (dwIndex, 1, W2CT(pszDescriptions[dwIndex]));
//#else
...
//#endif						
						...
						if (!pLookupErrs || FAILED(pLookupErrs[dwIndex]))
						{
							// No lookup item ID, so place "N/A" in list control:
							//lstrcpyn (szBuffer, _T("N/A"), sizeof (szBuffer) / sizeof (TCHAR));
							CString str("N/A");
							pszLookupIDs[dwIndex] = T2W("N/A");
						}
						pList->SetItemText (dwIndex, 3, W2CT(pszLookupIDs[dwIndex]));
					}
				}
			}
		}
