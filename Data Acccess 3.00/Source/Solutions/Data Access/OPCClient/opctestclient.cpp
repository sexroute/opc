...
void CKApp::SetAuth()
{
	...
	CString lstrUser = _T("eric");
	CString lstrPassword = _T("aA977910");
	CKServer::UserName(lstrUser);
	CKServer::Password(lstrPassword);

	SecureZeroMemory(&this->m_oWINNT_AuthIdentity, sizeof(this->m_oWINNT_AuthIdentity));
	WCHAR * lpUser = T2W(lstrUser);
	WCHAR * lpPassword =T2W(lstrPassword);
...
SEC_WINNT_AUTH_IDENTITY_W         * CKApp::GetAuthIdentity()
	{
		return & this->m_oWINNT_AuthIdentity;
	}
