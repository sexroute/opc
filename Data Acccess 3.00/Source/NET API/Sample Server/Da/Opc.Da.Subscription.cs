...
namespace Opc.Da.SampleServer
{
	...
	/// A class that manages a subscription for a set of items.
	/// </summary>
	public class  Subscription : Opc.Da.ISubscription
	{	
		/// <summary>
		...
		public void Update(long ticks, int interval)
		{
			lock (this)
			{
				...
				if (m_tickOffset < 0)
				{
					callbackRequired = m_tickOffset == -1;
					m_tickOffset     = ticks;
				}
				...
				if (callbackRequired && changedItems > 0)
				{
				...
				{
					long delta = DateTime.Now.Ticks - m_lastUpdate.Ticks;
