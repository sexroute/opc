...
{	
	...
	{	
		...
		/// <remarks/>
		public void BrowseOPCItemIDs(OpcRcw.Da.OPCBROWSETYPE dwBrowseFilterType, string szFilterCriteria, short vtDataTypeFilter, int dwAccessRightsFilter, out OpcRcw.Da.IEnumString ppIEnumString)
		{
			lock (this)
			{
				try
				{
					...
					// create enumerator.
					ppIEnumString = (OpcRcw.Da.IEnumString)new EnumString(hits);
				}
				catch (Exception e)
				{
					throw CreateException(e);
		...
		public void RemoteNext(int cConnections, IConnectionPoint[] ppCP, out int pcFetched)
		{
			lock (this)
			{
				try
				{
					...
					{
						ppCP[ii] = (IConnectionPoint)m_connectionPoints[m_index+ii];
						pcFetched++;
					}
		...
		public void RemoteNext(int celt, object[] rgelt, out int pceltFetched)
		{
			lock (this)
			{
				try
				{
					...
					for (int ii = 0; ii < m_unknowns.Count - m_index && ii < rgelt.Length; ii++)
					{
						rgelt[ii] = m_unknowns[m_index+ii];
						pceltFetched++;
					}
		...
		public void RemoteNext(int celt, string[] rgelt, out int pceltFetched)
		{
			lock (this)
			{
				try
				{
					...
					for (int ii = 0; ii < m_strings.Count - m_index && ii < rgelt.Length; ii++)
					{
						rgelt[ii] = (string)m_strings[m_index+ii];
						pceltFetched++;
					}
