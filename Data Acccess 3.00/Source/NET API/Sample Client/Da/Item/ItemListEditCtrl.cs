...
namespace Opc.Da.SampleClient
{
	...
	public class ItemListEditCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		private Item m_template = null;

		/// <summary>
		/// Initializes the control with the specified template item.
		/// </summary>
		public void Initialize(Item template)
		{
			// clear existing items.
			ItemListLV.Items.Clear();

			// create template item.
			m_template = (template != null)?(Item)template.Clone():new Item();
		...
		/// </summary>
		public void Initialize(Item template, Item[] items)
		{
			Initialize(template);
		...
		/// </summary>
		private object GetFieldValue(Item item, int fieldID)
		{
		...
		public Item[] GetItems()
		{
			...
			foreach (ListViewItem listItem in ItemListLV.Items)
			{
				...
				// create copy of item.
				Item item = (Item)((Item)listItem.Tag).Clone();
		...
		/// </summary>
		private void EditTemplate(Item template)
		{			
			// prompt user to edit the template.
			Item[] templates = new ItemListEditDlg().ShowDialog(new Item[] { template }, IsReadItem, false);
			...
			{
				if (item.Tag != null && item.Tag.GetType() == typeof(Item))
				{
		...
		{			
			...
			Item[] items = new ItemListEditDlg().ShowDialog((Item[])itemList.ToArray(typeof(Item)), IsReadItem, false);
		...
		private void NewMI_Click(object sender, System.EventArgs e)
		{
			...
			{
				template = (Item)((Item)ItemListLV.SelectedItems[0].Tag).Clone();
			}
			
			// prompt user to edit new item.
			Item[] items = new ItemListEditDlg().ShowDialog(new Item[] { template }, IsReadItem, true);
