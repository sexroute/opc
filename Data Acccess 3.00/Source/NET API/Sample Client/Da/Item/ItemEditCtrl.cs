...
namespace Opc.Da.SampleClient
{
	...
	public class ItemEditCtrl : System.Windows.Forms.UserControl, IEditObjectCtrl
	{
		...
		private Opc.SampleClient.DataTypeCtrl ReqTypeCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.ReqTypeCTRL = new Opc.SampleClient.DataTypeCtrl();
		...
		/// </summary>
		private ItemIdentifier m_identifier = null;
		...
		public object Get()
		{
			Opc.Da.Item item = new Opc.Da.Item(m_identifier);
		...
		public void Set(object value)
		{
			...
			Opc.Da.Item item = (Opc.Da.Item)value;
		...
		/// </summary>
		private Opc.Cpx.ComplexItem m_item = null;

		/// <summary>
		/// Initializes the control with information specified to a complex item. 
		/// </summary>
		private void SetComplexItem(ItemIdentifier itemID)
		{
			m_item = ComplexTypeCache.GetComplexItem(itemID);
			...
			Opc.Cpx.ComplexItem[] conversions = m_item.GetRootItem().GetTypeConversions(ComplexTypeCache.Server);
			...
			{
				Opc.Cpx.ComplexItem item = m_item;

				if (item.UnfilteredItemID != null)
				{
					item = ComplexTypeCache.GetComplexItem(item.UnfilteredItemID);
				}

				foreach (ComplexItem conversion in conversions)
				{
		...
		/// </summary>
		private void GetComplexItem(ItemIdentifier itemID)
		{
			if (m_item == null) { return; }

			ComplexItem item = m_item;
			...
			if (!DataFilterSpecifiedCB.Checked || !DataFilterSpecifiedCB.Enabled || DataFilterTB.Text == "")
			{
				if (m_item.UnfilteredItemID != null)
				{
					m_item.UpdateDataFilter(ComplexTypeCache.Server, "");
					
					if (m_item.UnconvertedItemID != null)
					{
						item = ComplexTypeCache.GetComplexItem(m_item.UnconvertedItemID);
					}
					else
					{
						item = ComplexTypeCache.GetComplexItem(m_item.UnfilteredItemID);
					}
				}
			}
			...
			else
			{
				...
				ComplexItem conversion = (ComplexItem)TypeConversionCB.SelectedItem;
			...
			if (DataFilterSpecifiedCB.Checked && DataFilterSpecifiedCB.Enabled && DataFilterTB.Text != "")
			{
				...
				if (item.UnfilteredItemID != null)
				{
					item.UpdateDataFilter(ComplexTypeCache.Server, DataFilterTB.Text);
				}
				else
				{
					// get the existing data filters.
					ComplexItem[] filters = item.GetDataFilters(ComplexTypeCache.Server);
					...
					do
					{
						...
						foreach (ComplexItem filter in filters)
						{
					...
					// create the filter.
					item = item.CreateDataFilter(ComplexTypeCache.Server, filterName, DataFilterTB.Text);
				}
			}
		...
		private void TypeConversionCB_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			ComplexItem item = null;

			if (TypeConversionSpecifiedCB.Checked)
			{
				item = (ComplexItem)TypeConversionCB.SelectedItem;
			}
			else
			{
				if (m_item != null)
