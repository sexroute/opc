...
namespace Opc.Da.SampleClient
{
	...
	{
		private Opc.Da.SampleClient.ItemEditCtrl ObjectCTRL;
		/// <summary>
		...
		{
			this.ObjectCTRL = new Opc.Da.SampleClient.ItemEditCtrl();
			this.SuspendLayout();
			// 
			// ObjectCTRL
			// 
		...
		/// </summary>
		public Item[] ShowDialog(Item[] items, bool isReadItems, bool allowEditItemID)
		{
			ObjectCTRL.IsReadItem      = isReadItems;
			ObjectCTRL.AllowEditItemID = allowEditItemID;

			if (items == null) items = new Item[] { (Item)ObjectCTRL.Create() };

			ArrayList results = base.ShowDialog((object[])items, !allowEditItemID);

			if (results != null && results.Count > 0)
			{
				return (Item[])results.ToArray(typeof(Item));
			}
