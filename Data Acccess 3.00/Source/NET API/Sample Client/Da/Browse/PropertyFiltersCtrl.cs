...
namespace Opc.Da.SampleClient
{
	...
	public class PropertyFiltersCtrl : System.Windows.Forms.UserControl
	{
		...
		public PropertyFiltersCtrl()
		{
			...
			PropertyDescription[] properties = PropertyDescription.Enumerate();
		...
		public PropertyID[] PropertyIDs
		{
			...
			{ 
				...
				foreach (PropertyDescription property in PropertyNamesLB.CheckedItems)
				{
					propertyIDs.Add(property.ID);
				}

				return (PropertyID[])propertyIDs.ToArray(typeof(PropertyID)); 
			}
			...
			{ 
				for (int ii = 0; ii < PropertyNamesLB.Items.Count; ii++)
				{
					...
					if (value != null)
					{
						PropertyDescription property = (PropertyDescription)PropertyNamesLB.Items[ii];

						foreach (PropertyID propertyID in value)
						{
