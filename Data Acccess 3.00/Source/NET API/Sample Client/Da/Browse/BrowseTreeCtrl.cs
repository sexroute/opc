...
namespace Opc.Da.SampleClient
{
	...
	/// </summary>
	public delegate void ServerPicked_EventHandler(Opc.Da.Server server);
	...
	public class BrowseTreeCtrl : System.Windows.Forms.UserControl
	{
		...
		public Opc.Da.Server SelectedServer 
		{
			get
			{ 
				Opc.Da.Server server = FindServer(BrowseTV.SelectedNode); 

				if (server != null)
				{
					return (Opc.Da.Server)server.Duplicate();
				}
		...
		private void Browse(TreeNode node)
		{
			...
			{
				// get the server for the current node.
				Opc.Da.Server server = FindServer(node);
		...
		private void PickNode(TreeNode node)
		{
			if (IsServerNode(node))
			{
				...
				{
					ServerPicked((Opc.Da.Server)node.Tag);
				}
			}
		...
		private void BrowseTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				PickMI.Enabled        = true;
				ConnectMI.Enabled     = !((Opc.Da.Server)clickedNode.Tag).IsConnected;
				DisconnectMI.Enabled  = ((Opc.Da.Server)clickedNode.Tag).IsConnected;
		...
		{
			new BrowseFiltersDlg().Show(MainForm.ActiveForm, m_filters, new BrowseFiltersChangedCallback(OnBrowseFiltersChanged));	
		}
