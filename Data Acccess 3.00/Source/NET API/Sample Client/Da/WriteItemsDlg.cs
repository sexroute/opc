...
namespace Opc.Da.SampleClient
{
	...
	public class WriteItemsDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Da.SampleClient.ItemValueListEditCtrl ItemsCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.ItemsCTRL = new Opc.Da.SampleClient.ItemValueListEditCtrl();
		...
		public void DoWrite()
		{
			try
			{
				...
				ItemValue[] items = ItemsCTRL.GetItems();
				...
				else
				{
					...
					foreach(ItemValue item in items) { item.ClientHandle = item.ItemName; }
				...
				{
					ItemValueResult value = new ItemValueResult(items[ii]);
				...
				// save results.
				m_values = (ItemValueResult[])values.ToArray(typeof(ItemValueResult));
		...
		{
			ItemsCTRL.AddItemWithDefaults(new ItemValue(itemID));
		}
