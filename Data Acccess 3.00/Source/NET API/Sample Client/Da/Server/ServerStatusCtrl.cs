...
namespace Opc.Da.SampleClient
{
	...
	public class ServerStatusCtrl : System.Windows.Forms.StatusBar
	{
		...
		private void UpdateTimer_Tick(object sender, System.EventArgs e)
		{
			try
			{
				GetStatusAsyncDelegate callback = new GetStatusAsyncDelegate(m_server.GetStatus);
		...
		private void OnGetStatus(IAsyncResult result)
		{
			...
			try
			{
				GetStatusAsyncDelegate callback = (GetStatusAsyncDelegate)result.AsyncState;

				ServerStatus status = callback.EndInvoke(result);
