	...
	public class UpdatesListViewCtrl : System.Windows.Forms.UserControl
	{
			...
			try
			{
				...
				foreach (ItemValueResult value in values)
				{
					...
					if (!KeepValuesMI.Checked)
					{
						...
						foreach (ListViewItem listItem in existingItemList)
						{
							ItemValueResult previous = (ItemValueResult)listItem.Tag;
		...
		private void OnKeepAlive(object subscription)
		{
			if (UpdateEvent != null)
			{
				...
				message += Opc.Convert.ToString(DateTime.Now);
				message += "\t";
		...
		/// </summary>
		private void AddValue(object subscriptionHandle, ItemValueResult item, ref ListViewItem replaceableItem)
		{
			string quality = "";
			...
			if (item.QualitySpecified)
			{
				...
				if (item.Quality.LimitBits != limitBits.none)
				{
					quality += ", ";
			...
			{
				ItemValueResult previous = (ItemValueResult)ItemListLV.Items[ii].Tag;
		...
		/// </summary>
		private string GetErrorText(object subscriptionHandle, ResultID resultID)
		{
		...
		private void ItemListLV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				if (clickedItem.Tag != null && clickedItem.Tag.GetType() == typeof(ItemValueResult))
				{
					ItemValueResult item = (ItemValueResult)clickedItem.Tag;

					if (item.Value != null)
					{
						ViewMI.Enabled = ((ComplexTypeCache.GetComplexItem(item) != null) || (item.Value.GetType().IsArray));
					}
				}
			}
		}
