...
namespace Opc.Da.SampleClient
{
	...
	{
		private Opc.Da.SampleClient.SubscriptionEditCtrl ObjectCTRL;
		/// <summary>
		...
		{
			this.ObjectCTRL = new Opc.Da.SampleClient.SubscriptionEditCtrl();
			this.SuspendLayout();
			// 
			// ObjectCTRL
			// 
		...
		/// </summary>
		public SubscriptionState ShowDialog(Opc.Da.Server server, SubscriptionState state)
		{
			ObjectCTRL.Server = server;

			if (state == null) state = (SubscriptionState)ObjectCTRL.Create();

			ArrayList results = ShowDialog(new object[] { state });

			if (results != null && results.Count == 1)
			{
				return (SubscriptionState)results[0];
			}
