...
namespace Opc.Da.SampleClient
{
	...
	public class SubscriptionCreateDlg : System.Windows.Forms.Form
	{
		...
		private Opc.Da.SampleClient.SubscriptionEditCtrl SubscriptionCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.SubscriptionCTRL = new Opc.Da.SampleClient.SubscriptionEditCtrl();
		...
		/// </summary>
		private ItemResult[] m_items = null;

		/// <summary>
		/// Prompts a user to create a new subscription with a modal dialog. 
		/// </summary>
		public Subscription ShowDialog(Opc.Da.Server server)
		{
		...
		private void DoCreate()
		{
			try
			{
				...
				SubscriptionState state = (SubscriptionState)SubscriptionCTRL.Get();

				state.ClientHandle = Guid.NewGuid().ToString();

				// create the subscription.
				m_subscription = (Opc.Da.Subscription)m_server.CreateSubscription(state);
