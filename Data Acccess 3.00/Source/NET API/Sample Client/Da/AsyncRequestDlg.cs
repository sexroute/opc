...
{	
	...
	public class AsyncRequestDlg : System.Windows.Forms.Form
	{
		...
		/// </summary>
		private ItemValue[] m_values = null;

		/// <summary>
		/// The results of the operation.
		/// </summary>
		private ItemIdentifier[] m_results = null;
		...
		/// </summary>
		public ItemValueResult[] ShowDialog(Opc.Da.Subscription subscription, Item[] items)
		{
			...
			// return results.
			return (ItemValueResult[])m_results;
		}

		/// <summary>
		/// Executes an asynchronous read and displays the results.
		/// </summary>
		public IdentifiedResult[] ShowDialog(Opc.Da.Subscription subscription, ItemValue[] values)
		{
			if (subscription == null) throw new ArgumentNullException("subscription");
			...
			// return results.
			return (IdentifiedResult[])m_results;
		}
		...
		private void BeginRequest()
		{
			...
			{				
				...
				if (m_items != null)
				{
					m_subscription.Read(m_items, ++m_handle, new ReadCompleteEventHandler(OnReadComplete), out m_request);
				}				

				// begin the asynchronous write request.
				else if (m_values != null)
				{
					m_subscription.Write(m_values, ++m_handle, new WriteCompleteEventHandler(OnWriteComplete), out m_request);
				}
		...
		private void CancelRequest()
		{
			try
			{
				if (m_request != null)
				{
					m_subscription.Cancel(m_request, new CancelCompleteEventHandler(OnCancelComplete));
				}				
			}
			catch (Exception e)
			{
		...
		private void OnReadComplete(object clientHandle, ItemValueResult[] results)
		{
			try
			{
				if (InvokeRequired)
				{
					BeginInvoke(new ReadCompleteEventHandler(OnReadComplete), new object[] { clientHandle, results });
					return;
				}
		...
		private void OnCancelComplete(object clientHandle)
		{
			if (InvokeRequired)
			{
				BeginInvoke(new CancelCompleteEventHandler(OnCancelComplete), new object[] { clientHandle });
				return;
			}
