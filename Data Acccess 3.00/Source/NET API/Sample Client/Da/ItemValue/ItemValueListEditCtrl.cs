...
namespace Opc.Da.SampleClient
{
	...
	public class ItemValueListEditCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		private ItemValue m_template = null;

		/// <summary>
		/// Initializes the control with the specified set of items.
		/// </summary>
		public void Initialize(Server server, ItemValue template)
		{
			...
			m_template = (template != null)?(ItemValue)template.Clone():new ItemValue();
		...
		/// Initializes the control with the specified set of items.
		/// </summary>
		public void Initialize(Server server, ItemValue template, Item[] items)
		{
			...
			if (items != null)
			{
				foreach (Item item in items) AddItemWithDefaults(new ItemValue(item));
			}
		...
		/// </summary>
		private void GetDefaultValues(ItemValue[] items, bool valuesOnly)
		{
			try
			{
				...
				ItemPropertyCollection[] propertyLists = m_server.GetProperties(
					items,
					new PropertyID[] { Property.DATATYPE, Property.QUALITY, Property.TIMESTAMP, Property.VALUE },
					true);
				...
				for (int ii = 0; ii < items.Length; ii++)
				{
					...
					items[ii].Quality   =  (Quality)propertyLists[ii][1].Value;
		...
		/// </summary>
		private void UpdateItem(ListViewItem listItem, ItemValue item)
		{
		...
		/// </summary>
		private object GetFieldValue(ItemValue item, int fieldID)
		{
			...
			switch (fieldID)
			{
				...
				case VALUE:
				{
					...
					if (fieldValue == null)	fieldValue = Opc.Convert.Clone(m_template.Value);
					break;
				}
		...
		/// </summary>
		public void AddItemWithDefaults(ItemValue item)
		{
			GetDefaultValues(new ItemValue[] { item }, ValuesOnlyMI.Checked);
			AddItem(item);
		}
		...
		/// </summary>
		private void AddItem(ItemValue item)
		{
		...
		public ItemValue[] GetItems()
		{
			...
			foreach (ListViewItem listItem in ItemListLV.Items)
			{
				...
				// create copy of item.
				ItemValue item = (ItemValue)((ItemValue)listItem.Tag).Clone();
				...
				item.Quality            = (field != null)?(Quality)field:Quality.Bad;
			...
			return (ItemValue[])items.ToArray(typeof(ItemValue));
		}	
		...
		/// </summary>
		private void EditTemplate(ItemValue template)
		{			
			// prompt user to edit the template.
			ItemValue[] templates = new ItemValueListEditDlg().ShowDialog(new ItemValue[] { template }, false);
			...
			{
				if (item.Tag != null && item.Tag.GetType() == typeof(ItemValue))
				{
			...
			// add items back.
			foreach (ItemValue item in items) AddItem(item);
		}
		...
		{			
			...
			ItemValue[] items = new ItemValueListEditDlg().ShowDialog((ItemValue[])itemList.ToArray(typeof(ItemValue)), false);
		...
		{
			ItemValue template = null;

			// copy the current selection.
			if (ItemListLV.SelectedItems.Count > 0)
			{
				template = (ItemValue)((ItemValue)ItemListLV.SelectedItems[0].Tag).Clone();
			}
			
			// prompt user to edit new item.
			ItemValue[] items = new ItemValueListEditDlg().ShowDialog(new ItemValue[] { template }, true);
		...
		private void InitMI_Click(object sender, System.EventArgs e)
		{
			...
			{
				if (listItem.Tag != null && listItem.Tag.GetType() == typeof(ItemValue))
				{
					if (listItem.Tag != m_template)	
					{
			...
			GetDefaultValues((ItemValue[])items.ToArray(typeof(ItemValue)), ValuesOnlyMI.Checked);
			
			// update list view.
			foreach (ListViewItem listItem in ItemListLV.SelectedItems) 
			{
				UpdateItem(listItem, (ItemValue)listItem.Tag);
			}
		...
		private void ValuesOnlyMI_Click(object sender, System.EventArgs e)
		{
			...
			if (ValuesOnlyMI.Checked)
			{
				...
				{
					// get item.
					ItemValue item = (ItemValue)listItem.Tag;
