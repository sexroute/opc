...
namespace Opc.Da.SampleClient
{
	...
	{
		private Opc.Da.SampleClient.ItemValueEditCtrl ObjectCTRL;
		/// <summary>
		...
		{
			this.ObjectCTRL = new Opc.Da.SampleClient.ItemValueEditCtrl();
			this.SuspendLayout();
			// 
			// ObjectCTRL
			// 
		...
		/// </summary>
		public ItemValue[] ShowDialog(ItemValue[] items, bool allowEditItemID)
		{
			ObjectCTRL.AllowEditItemID = allowEditItemID;

			if (items == null) items = new ItemValue[] { (ItemValue)ObjectCTRL.Create() };

			ArrayList results = base.ShowDialog((object[])items, !allowEditItemID);

			if (results != null && results.Count > 0)
			{
				return (ItemValue[])results.ToArray(typeof(ItemValue));
			}
