...
{	
	...
	public class OptionsEditDlg : System.Windows.Forms.Form
	{
		...
		private Opc.SampleClient.LocaleCtrl LocaleCTRL;
		...
		private void InitializeComponent()
		{
			...
			this.LocaleCTRL = new Opc.SampleClient.LocaleCtrl();
		...
		/// </summary>
		public void ShowDialog(Opc.Da.Server server)
		{
		...
		/// Prompts user to edit request option parameters in a modal dialog.
		/// </summary>
		public void ShowDialog(Opc.Da.Subscription subscription)
		{
		...
		/// Prompts user to edit request option parameters in a modal dialog.
		/// </summary>
		private void ShowDialog(Opc.Da.Server server, Opc.Da.Subscription subscription)
		{
			...
			ItemNameCB.Checked       = ((filters & (int)ResultFilter.ItemName)       != 0);
			ItemPathCB.Checked       = ((filters & (int)ResultFilter.ItemPath)       != 0);
			ClientHandleCB.Checked   = ((filters & (int)ResultFilter.ClientHandle)   != 0);
			ItemTimeCB.Checked       = ((filters & (int)ResultFilter.ItemTime)       != 0);
			ErrorTextCB.Checked      = ((filters & (int)ResultFilter.ErrorText)      != 0);
			DiagnosticInfoCB.Checked = ((filters & (int)ResultFilter.DiagnosticInfo) != 0);
			...
			while (ShowDialog() == DialogResult.OK)
			{
				...
				try
				{
					...
					else
					{
						...
						state.Locale = locale;
						subscription.ModifyState((int)StateMask.Locale, state);
					}
				}
				catch (Exception e)
				{
				...
				filters |= (ItemNameCB.Checked)?(int)ResultFilter.ItemName:0;
				filters |= (ItemPathCB.Checked)?(int)ResultFilter.ItemPath:0;
				filters |= (ClientHandleCB.Checked)?(int)ResultFilter.ClientHandle:0;
				filters |= (ItemTimeCB.Checked)?(int)ResultFilter.ItemTime:0;
				filters |= (ErrorTextCB.Checked)?(int)ResultFilter.ErrorText:0;
				filters |= (DiagnosticInfoCB.Checked)?(int)ResultFilter.DiagnosticInfo:0;
