...
namespace Opc.Da.SampleClient
{
	...
	public class ResultListViewCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public void Initialize(Opc.Da.Server server, string locale, IdentifiedResult[] results)
		{
			m_server = server;
			m_locale = locale;
		...
		/// </summary>
		public void Initialize(Opc.Da.Server server, string locale, ItemResult[] results)
		{
			m_server = server;
			m_locale = locale;
		...
		/// </summary>
		public void Initialize(Opc.Da.Server server, string locale, ItemValueResult[] results)
		{
			m_server = server;
			m_locale = locale;
		...
		{
			if (item.GetType() == typeof(IdentifiedResult)) return GetFieldValue((IdentifiedResult)item, fieldID);
			if (item.GetType() == typeof(ItemResult))       return GetFieldValue((ItemResult)item, fieldID);
			if (item.GetType() == typeof(ItemValueResult))  return GetFieldValue((ItemValueResult)item, fieldID);
		...
		/// Returns the value of the specified field.
		/// </summary>
		private object GetFieldValue(IdentifiedResult item, int fieldID)
		{
			switch (fieldID)
			{
		...
		/// Returns the value of the specified field.
		/// </summary>
		private object GetFieldValue(ItemResult item, int fieldID)
		{
		...
		/// Returns the value of the specified field.
		/// </summary>
		private object GetFieldValue(ItemValueResult item, int fieldID)
		{
			object fieldValue = null;
			...
			switch (fieldID)
			{
				...
				case QUALITY_BITS:  { return (item.QualitySpecified && item.Quality.QualityBits != qualityBits.good)?item.Quality.QualityBits:fieldValue; }
				case LIMIT_BITS:    { return (item.QualitySpecified && item.Quality.LimitBits != limitBits.none)?item.Quality.LimitBits:fieldValue; }
		...
		private void AddResult(object item)
		{
			...
			listItem.SubItems.Add(Opc.Convert.ToString(GetFieldValue(item, DATA_TYPE)));
			...
			listItem.SubItems.Add(Opc.Convert.ToString(GetFieldValue(item, ERROR)));
			listItem.SubItems.Add(Opc.Convert.ToString(GetFieldValue(item, CLIENT_HANDLE)));
			listItem.SubItems.Add(Opc.Convert.ToString(GetFieldValue(item, SERVER_HANDLE)));
		...
		/// </summary>
		private string GetErrorText(ResultID resultID)
		{
			if (ShowErrorTextMI.Checked)
			{
				// display nothing for ok results.
				if (resultID == ResultID.S_OK) return "";
		...
		private void ViewMI_Click(object sender, System.EventArgs e)
		{
			if (ItemListLV.SelectedItems.Count > 0)
			{
				...
				if (tag != null && tag.GetType() == typeof(ItemValueResult))
				{
					ItemValueResult item = (ItemValueResult)tag;
		...
		private void ItemListLV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				ViewMI.Enabled = (clickedItem.Tag != null && clickedItem.Tag.GetType() == typeof(ItemValueResult));
			}
		}
		...
		private void ShowErrorTextMI_Click(object sender, System.EventArgs e)
		{
			...
			foreach (ListViewItem listItem in ItemListLV.Items)
			{
				...
				if (listItem.Tag.GetType() == typeof(IdentifiedResult))
				{
					errorText = GetErrorText(((IdentifiedResult)listItem.Tag).ResultID);
				}

				else if (listItem.Tag.GetType() == typeof(ItemResult))
				{
					errorText = GetErrorText(((ItemResult)listItem.Tag).ResultID);
				}

				else if (listItem.Tag.GetType() == typeof(ItemValueResult))
				{
					errorText = GetErrorText(((ItemValueResult)listItem.Tag).ResultID);
				}
