...
namespace Opc.SampleClient
{
	...
	/// </summary>
	public delegate void ServerPicked_EventHandler(Opc.Server server);
	...
	public class BrowseTreeCtrl : System.Windows.Forms.UserControl
	{
		...
		/// </summary>
		public Opc.Server SelectedServer 
		{
			get
			{ 
				Opc.Server server = FindServer(BrowseTV.SelectedNode); 

				if (server != null)
				{
					return (Opc.Server)server.Duplicate();
				}
		...
		public ItemIdentifier SelectedItem 
		{
			...
			{			
				...
				{
					BrowseElement element = (BrowseElement)node.Tag;

					if (element.IsItem)
					{
						return new ItemIdentifier(element.ItemPath, element.ItemName);
					}
				}

				if (IsItemPropertyNode(node))
				{
					ItemProperty property = (ItemProperty)node.Tag;

					if (property.ItemName != null && ItemPicked != null)
					{
						return new ItemIdentifier(property.ItemPath, property.ItemName);
					}
				}
		...
		/// </summary>
		public ConnectData SelectedConnectData {get{ return FindConnectData(BrowseTV.SelectedNode); }}
		...
		public void ShowAllServers(IDiscovery discovery, Specification specification, BrowseFilters filters)
		{
			...
			m_filters       = (filters == null)?new BrowseFilters():filters;
		...
		/// </summary>
		public void ShowSingleServer(Opc.Server server, BrowseFilters filters)
		{
			...
			m_singleServer                    = new TreeNode(server.Name);
		...
		private void Connect(TreeNode node)
		{
			try
			{
				...
				Opc.Server server = (Opc.Server)node.Tag;
		...
		private ConnectData FindConnectData(TreeNode node)
		{
			...
			if (BrowseTV.Tag != null && BrowseTV.Tag.GetType() == typeof(ConnectData))
			{
				return (ConnectData)BrowseTV.Tag;
			}
		...
		private void BrowseServers(TreeNode node)
		{
			try
			{
				...
				// find the servers.
				Opc.Server[] servers = m_discovery.GetAvailableServers(m_specification, host, connectData);
		...
		private void Browse(TreeNode node)
		{
			...
			{
				// get the server for the current node.
				Opc.Server selection = FindServer(node);

				if (!typeof(Opc.Da.Server).IsInstanceOfType(selection))
				{
					return;
				}

				Opc.Da.Server server = (Opc.Da.Server)selection;
				...
				BrowseElement[] elements = server.Browse(itemID, m_filters, out position);
				...
				{
					if (node.Tag.GetType() == typeof(BrowseElement))
					{
						ElementSelected((BrowseElement)node.Tag);
					}
					else
					{
		...
		private void GetProperties(TreeNode node)
		{
			try
			{
				...
				// begin a browse.			
				ItemIdentifier itemID = new ItemIdentifier(element.ItemPath, element.ItemName);

				ItemPropertyCollection[] propertyLists = server.GetProperties(
				...
				{
					foreach (ItemPropertyCollection propertyList in propertyLists)
					{
						...
						element.Properties = (ItemProperty[])propertyList.ToArray(typeof(ItemProperty));
					}
				}
		...
		private bool IsServerNode(TreeNode node)
		{
			...
			return typeof(Opc.Server).IsInstanceOfType(node.Tag);
		}
		...
		{
			if (node == null || node.Tag == null) return false;
			return (node.Tag.GetType() == typeof(ItemProperty));
		}

		/// <summary>
		/// Finds the server for the specified node.
		/// </summary>
		private Opc.Server FindServer(TreeNode node)
		{
			if (node != null)
			{
				if (IsServerNode(node))
				{
					return (Opc.Server)node.Tag;
				}
		...
		private void PickNode(TreeNode node)
		{
			if (IsServerNode(node))
			{
				...
				{
					ServerPicked((Opc.Server)node.Tag);
				}
			}
			...
			else if (IsBrowseElementNode(node))
			{
				...
				{
					ItemPicked(new ItemIdentifier(element.ItemPath, element.ItemName));
				}
			}
			...
			else if (IsItemPropertyNode(node))
			{
				...
				if (property.ItemName != null && ItemPicked != null)
				{
					ItemPicked(new ItemIdentifier(property.ItemPath, property.ItemName));
				}
			}
		}
		...
		{		
			...
			if (m_discovery != null) m_discovery.Dispose();
		...
		{		
			...
			{
				Opc.Server server = (Opc.Server)parent.Tag;
		...
		private void BrowseTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			{
				PickMI.Enabled        = true;
				ConnectMI.Enabled     = !((Opc.Server)clickedNode.Tag).IsConnected;
				DisconnectMI.Enabled  = ((Opc.Server)clickedNode.Tag).IsConnected;
			...
			if (IsBrowseElementNode(clickedNode))
			{
				...
				EditFiltersMI.Enabled     = true;
				RefreshMI.Enabled         = true;
				ViewComplexTypeMI.Enabled = (ComplexTypeCache.GetComplexItem((BrowseElement)clickedNode.Tag) != null);
				return;
			}
		}	
		...
		private void OnBrowseFiltersChanged(BrowseFilters filters)
		{
			...
			{
				BrowseElement element = (BrowseElement)BrowseTV.SelectedNode.Tag;
		...
		private void SetLoginMI_Click(object sender, System.EventArgs e)
		{
			...
			{ 
				ConnectData connectData = (ConnectData)node.Tag;

				if (connectData == null)
				{
					node.Tag = connectData = new ConnectData(null);
				}
