...
namespace Opc.SampleClient
{
	...
	public class EditComplexValueDlg : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.FormattedPN.DockPadding.All = 4;
			...
			this.TypeDictionaryMI.Text = "Type Dictionary";
			...
			this.SchemaPN.DockPadding.All = 4;
			...
			this.TypeDictionaryTB.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			...
			this.TypeDictionaryTB.Text = "";
			this.TypeDictionaryTB.WordWrap = false;
			// 
			// TypeDescriptionTB
			// 
			this.TypeDescriptionTB.Dock = System.Windows.Forms.DockStyle.Fill;
			this.TypeDescriptionTB.Font = new System.Drawing.Font("Microsoft Sans Serif", 8.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			...
			this.TypeDescriptionTB.Text = "";
			this.TypeDescriptionTB.WordWrap = false;
			// 
			// RawTB
			// 
			this.RawTB.Dock = System.Windows.Forms.DockStyle.Fill;
			this.RawTB.Font = new System.Drawing.Font("Courier New", 9.75F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
		...
		/// </summary>
		private ComplexItem m_item = null;
		...
		/// </summary>
		private ComplexValue m_complexValue = null;
		...
		private class BinaryValue
		{
			...
			public ComplexValue Init(object rawValue, string dictionary, string typeID)
			{
				XmlTextReader reader     = new XmlTextReader(dictionary, XmlNodeType.Document, null);
				XmlSerializer serializer = new XmlSerializer(typeof(TypeDictionary), Namespace.OPC_BINARY);
			...
			public ComplexValue Parse(byte[] data)
			{
				return new Opc.Cpx.BinaryReader().Read(data, m_dictionary, m_typeID);
			}

			// serializes the buffer according to the schema.
			public byte[] Parse(ComplexValue complexValue)
			{
				return new Opc.Cpx.BinaryWriter().Write(complexValue, m_dictionary, m_typeID);
			}
		}
		...
		private void UpdateView()
		{
			...
			if (TypeTV.Nodes.Count > 0)
			{
				...
				ShowComplexValue((ComplexValue)TypeTV.SelectedNode.Tag);
			}
		}
		...
		/// </summary>
		private void ShowFormattedValue(TreeNodeCollection nodes, ComplexValue complexValue, int index)
		{
			...
			if (complexValue.Value.GetType() == typeof(ComplexValue[]))
			{
				int ii = 0;

				foreach (ComplexValue element in (Array)complexValue.Value)
				{
		...
		/// </summary>
		void ShowComplexValue(ComplexValue complexValue)
		{
			...
			if (!complexValue.Value.GetType().IsArray || complexValue.Value.GetType() == typeof(byte[]))
			{
				...
				listItem.SubItems.Add(Opc.Convert.ToString(complexValue.Value.GetType()));
				listItem.SubItems.Add(Opc.Convert.ToString(complexValue.Value));
			...
			for (int ii = 0; ii < fields.Count; ii++)
			{
				...
				if (fields[ii].GetType() == typeof(ComplexValue))
				{					
					ComplexValue elementValue = (ComplexValue)fields[ii];

					listItem = new ListViewItem(elementValue.Name, Resources.IMAGE_EXPLODING_BOX);
				
					// display a complex value field.
					if (elementValue.Value.GetType() == typeof(ComplexValue[]))
					{
						listItem.SubItems.Add(Opc.Convert.ToString(elementValue.Type));
					...
					{
						listItem.SubItems.Add(Opc.Convert.ToString(elementValue.Type));
						listItem.SubItems.Add(Opc.Convert.ToString(elementValue.Value));
					}
				...
				else
				{
					...
					listItem.SubItems.Add(Opc.Convert.ToString(fields[ii].GetType()));
					listItem.SubItems.Add(Opc.Convert.ToString(fields[ii]));
		...
		/// </summary>
		private void EditComplexValue(ComplexValue complexValue)
		{
			object value = null;
		...
		private void TypeTV_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			if (node.Tag != null && node.Tag.GetType() == typeof(ComplexValue))
			{
				ShowComplexValue((ComplexValue)node.Tag);
			}
		}
		...
		private void TypeTV_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if (clickedNode.Tag == null || clickedNode.Tag.GetType() != typeof(ComplexValue))
			{
				return;
			}

			ComplexValue value = (ComplexValue)clickedNode.Tag;

			if (value.Value.GetType() == typeof(ComplexValue[]))
			{
				return;
			}
		...
		{			
			...
			if (node != null && node.Tag != null && node.Tag.GetType() == typeof(ComplexValue))
			{
				EditComplexValue((ComplexValue)node.Tag);
			}
		}
	}
}
