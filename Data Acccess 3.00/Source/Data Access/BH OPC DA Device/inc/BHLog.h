...
#define gc_cTimeFormat          _T("%Y-%m-%d %H:%M:%S")

#define BHLOG(level, info) \
	do {	CString sLine; \
	sLine.Format(_T("%s@%d"), __FUNCTION__, __LINE__); \
...
do {\
	...
	logStr.append(STR(_T("[%d]=%f"), ArrayNum, (double)ArrayData[0])); \
...
#define BHLOG_ID(id) BHLOG_DEBUG(STR(_T("%s(%d)"), # id, id))
