...
HRESULT CBHOpcDaDeviceItem::GetAvailableProperties(
	...
	)
{
	...
	return GetAvailableProperties(cItemPath, cIDs, bReturnValues, cProperties);
}
	...
	)
{
	...
	for (UINT ii = 0; ii < cProperties.GetSize(); ii++)
	{
		...
		if (bReturnValues)
		{
			hResult = Read(cItemPath, dwID, cProperties[ii]->GetValue());
		}
		else
		{
...
// Read
HRESULT CBHOpcDaDeviceItem::Read(
							   const COpcString&     cItemPath,
							   DWORD                 dwPropertyID,
							   VARIANT&              cValue, 
							   ...
							   )
{
	...
	case OPC_PROPERTY_VALUE:
		{
			return Read(pItem, cItemPath, cValue, pftTimestamp, pwQuality);
		}
							   ...
							   )
{
	...
	COpcDaTypeDictionary* pDictionary = ::GetCache().GetTypeDictionary(pItem->DictionaryItemID);
...
// Write
HRESULT CBHOpcDaDeviceItem::Write(
								const COpcString&     cItemPath,
								bool                  bInternalWrite,
								DWORD                 dwPropertyID,
								...
								)
{
	...
	if (dwPropertyID == NULL || dwPropertyID == OPC_PROPERTY_VALUE)
	{
		...
		return Write(pItem, cItemPath, cValue, pftTimestamp, pwQuality);
	}
...
// WriteFilter
HRESULT CBHOpcDaDeviceItem::WriteFilter(
									  COpcDaComplexItem* pItem,
									  const COpcString&  cItemPath,
									  const VARIANT&     cValue
									  )
{
	...
	if (!AddItemAndLink(cNewItemPath))
	{
		return E_FAIL;
	}
								...
								)
{
	...
	if (IsFilterItem(cItemPath))
	{
		return WriteFilter(pItem, cItemPath, cValue);
	}
...
// Read
bool CBHOpcDaDeviceItem::Read(COpcXmlElement& cElement)
{
	Clear();
	...
	{
		COpcDaTypeDictionary* pDictionary = ::GetCache().CreateTypeDictionary(cDictionary);
		...
		if (pDictionary != NULL)
		{
			...
			COpcDaTypeDictionary* pMapping = ::GetCache().CreateXmlSchemaMapping(cDictionary);
...
// Write
bool CBHOpcDaDeviceItem::Write(COpcXmlElement& cElement)
{
	OPC_ASSERT(false);  
...
// FindItem
COpcDaComplexItem* CBHOpcDaDeviceItem::FindItem(const COpcString& cItemPath)
{
...
// IsFilterItem
bool CBHOpcDaDeviceItem::IsFilterItem(const COpcString& cItemPath)
{
	return (cItemPath.Find(CPX_DATA_FILTERS) != -1);
}

// GetFilterName
COpcString CBHOpcDaDeviceItem::GetFilterName(const COpcString& cItemPath)
{
...
// GetItemID
COpcString CBHOpcDaDeviceItem::GetItemID(const COpcString& cItemPath)
{
...
// AddItemAndLink
bool CBHOpcDaDeviceItem::AddItemAndLink(const COpcString& cItemPath)
{
	if (!::GetCache().AddItemAndLink(GetItemID(cItemPath)))
	{
		return false;
	}
...
// RemoveItemAndLink
void CBHOpcDaDeviceItem::RemoveItemAndLink(const COpcString& cItemPath)
{
	::GetCache().RemoveItemAndLink(GetItemID(cItemPath));
}

// BuildAddressSpace
bool CBHOpcDaDeviceItem::BuildAddressSpace()
{      
	if (!::GetCache().AddItemAndLink(m_cItemID))
	{
		return false;
	}
	...
	{
		if (!AddItemAndLink(CPX_DATA_FILTERS))
		{
			return false;
		}
	}
...
// ClearAddressSpace
void CBHOpcDaDeviceItem::ClearAddressSpace()
{
	if (m_pNativeFormat != NULL)
	{
		if (m_pNativeFormat->EnableFilters)
		{
			...
			RemoveItemAndLink(CPX_DATA_FILTERS);
		}
		...
		while (pos != NULL)
		{
			...
			RemoveItemAndLink(cTypeName);
		}
	}

	::GetCache().RemoveItemAndLink(m_cItemID);
}

// Parameters Constructor
CBHOpcDaDeviceItem::Parameters::Parameters()
{		
	uTicks         = 0;
	uInterval      = 1;
	uPeriod        = 0;
...
CBHOpcDaDeviceItem::Parameters::Parameters(const Parameters& cParameters)
{		
...
// Calculate
OpcXml::SByte CBHOpcDaDeviceItem::Calculate(OpcXml::SByte cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::Short CBHOpcDaDeviceItem::Calculate(OpcXml::Short cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::Int CBHOpcDaDeviceItem::Calculate(OpcXml::Int cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::Decimal CBHOpcDaDeviceItem::Calculate(OpcXml::Decimal cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::Long CBHOpcDaDeviceItem::Calculate(OpcXml::Long cValue, Parameters& cParameters)
{
...
// Calculate
OpcXml::Byte CBHOpcDaDeviceItem::Calculate(OpcXml::Byte cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::UShort CBHOpcDaDeviceItem::Calculate(OpcXml::UShort cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::UInt CBHOpcDaDeviceItem::Calculate(OpcXml::UInt cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::DateTime CBHOpcDaDeviceItem::Calculate(OpcXml::DateTime cValue, Parameters& cParameters)
{	
	SET_MAX(cParameters, QNAN);
	SET_MIN(cParameters, QNAN);
...
// Calculate
OpcXml::ULong CBHOpcDaDeviceItem::Calculate(OpcXml::ULong cValue, Parameters& cParameters)
{
	// determine upper and lower bounds.
...
// Calculate
OpcXml::Float CBHOpcDaDeviceItem::Calculate(OpcXml::Float cValue, Parameters& cParameters)
{	
...
// Calculate
OpcXml::Double CBHOpcDaDeviceItem::Calculate(OpcXml::Double cValue, Parameters& cParameters)
{
	// determine upper and lower bounds.
...
// Calculate
OpcXml::Boolean CBHOpcDaDeviceItem::Calculate(OpcXml::Boolean cValue, Parameters& cParameters)
{
	return !cValue;
}

// Calculate
OpcXml::String CBHOpcDaDeviceItem::Calculate(OpcXml::String& cValue, Parameters& cParameters)
{
	double dblFraction = (double)((cParameters.uTicks*cParameters.uInterval)%cParameters.uPeriod);	
	dblFraction /= cParameters.uPeriod;
...
// Update
void CBHOpcDaDeviceItem::Update(LONGLONG uTicks, UINT uInterval)
{
...
// Init
void CBHOpcDaDeviceItem::Init(OpcXml::AnyType& cValue, Parameters& cParameters)
{
...
// Update
bool CBHOpcDaDeviceItem::Update(OpcXml::AnyType& cValue, Parameters cParameters, LONGLONG uTicks, UINT uInterval)
{
...
// CheckRange
bool CBHOpcDaDeviceItem::CheckRange(const VARIANT& cValue)
{
	if (m_eEUType == OPC_ANALOG)
	{
...
// ValidatePropertyID
HRESULT CBHOpcDaDeviceItem::ValidatePropertyID(
	COpcDaComplexItem* pItem,
	const COpcString&  cItemPath, 
	DWORD              dwID, 
	int                iAccessRequired
...
BOOL CBHOpcDaDeviceItem::RealSample() const
{
	return m_bRealSample;
}

void CBHOpcDaDeviceItem::RealSample( BOOL val )
{
	m_bRealSample = val;
}

void CBHOpcDaDeviceItem::UpdateBHDevice( LONGLONG uTicks, UINT uInterval,OpcXml::AnyType&  arefNewValue)
{
	// build simulation parameter lists.
	Parameters cParameters;
...
bool CBHOpcDaDeviceItem::UpdateBHDevice( OpcXml::AnyType& cValue, 
										 Parameters cParameters, 
										 LONGLONG uTicks, 
										 UINT uInterval,
										 OpcXml::AnyType& arefNewValue)
{
	// update parameters from current schema.
	Init(m_cValue, cParameters);
	...
	{	switch (cValue.eType)
		{
		case OpcXml::XML_BOOLEAN:  { cValue.boolValue     = arefNewValue.boolValue;  break; }
		case OpcXml::XML_SBYTE:    { cValue.sbyteValue    = arefNewValue.sbyteValue; break; }
		case OpcXml::XML_BYTE:     { cValue.byteValue     = arefNewValue.byteValue; break; }
		case OpcXml::XML_SHORT:    { cValue.shortValue    = arefNewValue.shortValue; break; }
		case OpcXml::XML_USHORT:   { cValue.ushortValue   = arefNewValue.ushortValue; break; }
		case OpcXml::XML_INT:      { cValue.intValue      = arefNewValue.intValue; break; }
		case OpcXml::XML_UINT:     { cValue.uintValue     = arefNewValue.uintValue; break; }
		case OpcXml::XML_LONG:     { cValue.longValue     = arefNewValue.longValue; break; }
		case OpcXml::XML_ULONG:    { cValue.ulongValue    = arefNewValue.ulongValue; break; }
		case OpcXml::XML_FLOAT:    { cValue.floatValue    = arefNewValue.floatValue; break; }
		case OpcXml::XML_DOUBLE:   { cValue.doubleValue   = arefNewValue.doubleValue; break; }
		case OpcXml::XML_DECIMAL:  { cValue.decimalValue  = arefNewValue.decimalValue; break; }
		case OpcXml::XML_DATETIME: { cValue.dateTimeValue = arefNewValue.dateTimeValue; break; }
		case OpcXml::XML_STRING:   { cValue.stringValue   = arefNewValue.stringValue; break; }
		}
