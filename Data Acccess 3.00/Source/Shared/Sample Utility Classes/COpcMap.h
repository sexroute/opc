...
// CONTENTS:
// 
// A template double linked list class.
//
...
public:
    ...
    {
        UINT uBin = CustomHashKey(cKey)%m_uTableSize;
...
private:
    ...
    COpcEntry* NewEntry(const KEY& cKey)
    {
        ...
        pEntry->cKey  = cKey;
        pEntry->uHash = CustomHashKey(cKey);
...
template<class KEY>
inline UINT CustomHashKey(const KEY& cKey)
{
...
template<> 
inline UINT CustomHashKey<LPCTSTR> (const LPCTSTR& tsKey)
{
    LPCTSTR key = tsKey;
...
template<> 
inline UINT CustomHashKey<COpcString> (const COpcString& cKey)
{
    LPCTSTR key = cKey;
    if (key == NULL) return -1;
